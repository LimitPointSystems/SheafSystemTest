 ...
 *
 * @version $RCSfile: SheafScopeAbstractTablePane.java,v $ $Revision: 1.26 $
 *
...
public abstract class SheafScopeAbstractTablePane extends JPanel
{
  ...
  public String [] getSubposetNames(abstract_poset_member xmbr)
  {
    try
    {
      ...
      subposet lsp = new subposet();

      // Get iterator over the subposets in subject.

      index_space_iterator lsp_itr = host.subposet_iterator();
      ...
      while(!lsp_itr.is_done())
      {
        lsp.attach_to_state(host, lsp_itr.id());

        result[index] = lsp.name();

        index++;
        lsp_itr.next();
      }

      // Detach so state won't be deleted when handle goes out of scope.
