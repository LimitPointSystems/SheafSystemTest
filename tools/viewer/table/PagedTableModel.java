...
/**
 *  Classname: PagedTableModel
 *
 *  @author $Author: jebutler $
 *
 *  @version $RCSfile: PagedTableModel.java,v $ $Revision: 1.14 $
 *
  ...
  public Object getValueAt(int pageRow, int column)
  {
    ...
    if(column == 0)
    {
      try
      {
        ...
        index_map seq_id_map = schema.discretization_id_space().id_map();
        index_map disc_id_map = schema.discretization().id_map();

        int top_id = seq_id_map.range_id(actualRow);

        int disc_id = disc_id_map.domain_id(top_id);
