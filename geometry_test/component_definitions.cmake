...
#
set(clusters coordinates general mesh_generators point_locators
...
function(add_library_targets)
    ...
    if(${USE_VTK})
     ...
     if(WIN64INTEL OR WIN64MSVC)
        ...
        add_dependencies(${${COMPONENT}_DYNAMIC_LIB} ${FIBER_BUNDLES_TEST_IMPORT_LIBS} ${GEOMETRY_IMPORT_LIBS})

        if(${USE_VTK})
            target_link_libraries(${${COMPONENT}_DYNAMIC_LIB} ${FIBER_BUNDLES_IMPORT_LIBS} ${GEOMETRY_IMPORT_LIBS} ${VTK_LIBS}) 
        else()
            #target_link_libraries(${${COMPONENT}_DYNAMIC_LIB} ${FIBER_BUNDLES_IMPORT_LIBS} ${GEOMETRY_IMPORT_LIBS})
            target_link_libraries(${${COMPONENT}_DYNAMIC_LIB} ${FIBER_BUNDLES_IMPORT_LIBS} ${GEOMETRY_IMPORT_LIBS})  
        endif() 
        ...
        if(${USE_VTK})
            target_link_libraries(${${COMPONENT}_SHARED_LIB} ${FIBER_BUNDLES_TEST_SHARED_LIBS} ${GEOMETRY_SHARED_LIBS} ${VTK_LIBS})
            target_link_libraries(${${COMPONENT}_STATIC_LIB} ${FIBER_BUNDLES_STATIC_LIBS} ${VTK_LIBS})    
        else()
            target_link_libraries(${${COMPONENT}_SHARED_LIB} ${FIBER_BUNDLES_TEST_SHARED_LIBS} ${GEOMETRY_SHARED_LIBS})
