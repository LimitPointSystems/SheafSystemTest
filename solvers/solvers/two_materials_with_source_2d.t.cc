...
// $RCSfile: two_materials_with_source_2d.t.cc,v $ $Revision: 1.9 $ $Date: 2012/03/01 00:41:39 $

//
// Copyright (c) 2012 Limit Point Systems, Inc.
//

/// @example two_materials_with_source_2d.t.cc
///
/// Test case with a source in a single element and two materials with
/// markedly different thermal conductivities.
///
...
///
/// Thermal conductivity, k = k0 for (0<x<A, 0<y<B) and k = k1 elsewhere.
/// (A = 3.0 approximately; A = (3*i_size/5)*(i_size/5.0))
/// (B = 3.0 approximately; B = (3*j_size/5)*(j_size/5.0))
///
/// k0 = 2.0
/// k1 = 0.2

/// @file
/// Implementation for test case two_materials_with_source_2d.t
...
int main(int argc, char* argv[])
{
  ...
  string filename = "two_materials_with_source_2d.hdf";

  //===========================================================================

  // Create a fiber_bundles_namespace and get read-write access.

  fiber_bundles_namespace ns("two_materials_with_source_2d.t");
                              ...
                              size_type xj_size)
{
  ...
  int disc_ct = conductivities.schema().discretization_ct();

  double dx = 5.0/xi_size;
  double dy = 5.0/xj_size;

  pod_index_type client_id = 0;

  for(int j=0; j<xj_size; ++j)
  {
    for(int i=0; i<xi_size; ++i)
    {
      dof_type dof = ((dx*i < 3.0) && (dy*j < 3.0)) ? 2.0 : 0.2;
      conductivities.put_fiber(client_id, &dof, sizeof(sec_at0::dof_type));

      //cout << "client_id = " << client_id << " -> " << dof << endl;
