...
function(add_library_targets)
    ...
    if(WIN64INTEL OR WIN64MSVC)
        ...
        add_dependencies(${${COMPONENT}_DYNAMIC_LIB} ${FIBER_BUNDLES_IMPORT_LIB})

        if(${USE_VTK})
            target_link_libraries(${${COMPONENT}_DYNAMIC_LIB} ${TETGEN_LIB} ${FIBER_BUNDLES_IMPORT_LIB} ${VTK_LIBS}) 
        else()
            target_link_libraries(${${COMPONENT}_DYNAMIC_LIB} ${TETGEN_LIB} ${SHEAVES_IMPORT_LIB} ${FIBER_BUNDLES_IMPORT_LIB})        
        endif() 
        ...
        add_dependencies(${${COMPONENT}_SHARED_LIB} ${FIBER_BUNDLES_SHARED_LIB})
        ...
        add_dependencies(${PROJECT_NAME}-shared-lib ${${COMPONENT}_SHARED_LIB} ${FIBER_BUNDLES_SHARED_LIB})
        add_dependencies(${PROJECT_NAME}-static-lib ${${COMPONENT}_STATIC_LIB} ${FIBER_BUNDLES_STATIC_LIB})
        
        if(${USE_VTK})        
            target_link_libraries(${${COMPONENT}_SHARED_LIB} ${FIBER_BUNDLES_SHARED_LIB} ${SHEAVES_SHARED_LIB} ${VTK_LIBS} ${TETGEN_LIB})
            target_link_libraries(${${COMPONENT}_STATIC_LIB} ${FIBER_BUNDLES_STATIC_LIB} ${SHEAVES_STATIC_LIB} ${VTK_LIBS} ${TETGEN_LIB})    
        else()
            target_link_libraries(${${COMPONENT}_SHARED_LIB} ${FIBER_BUNDLES_SHARED_LIB} ${SHEAVES_SHARED_LIB} ${TETGEN_LIB})
            target_link_libraries(${${COMPONENT}_STATIC_LIB} ${FIBER_BUNDLES_STATIC_LIB} ${SHEAVES_STATIC_LIB} ${TETGEN_LIB})
        endif()
    endif()
...
function(add_bindings_targets)
    ...
    if((LINUX64INTEL OR LINUX64GNU) AND SWIG_FOUND)
        if(${USE_VTK}) 
            link_directories(${JDK_LIB_DIR} ${JVM_LIB_DIR} ${XAWT_LIB_DIR} ${VTK_LIB_DIR})
        else()
        ...
        add_dependencies(${${COMPONENT}_JAVA_BINDING_LIB} ${FIBER_BUNDLES_JAVA_BINDING_LIB} ${${COMPONENT}_SHARED_LIB})
        ...
        if(${USE_VTK}) 
            target_link_libraries(${${COMPONENT}_JAVA_BINDING_LIB} ${JDK_LIB_OPTIONS} ${HDF5_LIBRARIES} ${VTK_LIBS} ${FIBER_BUNDLES_JAVA_BINDING_LIB} ${${COMPONENT}_SHARED_LIB}) 
        else()
            target_link_libraries(${${COMPONENT}_JAVA_BINDING_LIB} ${JDK_LIB_OPTIONS} ${HDF5_LIBRARIES} ${FIBER_BUNDLES_JAVA_BINDING_LIB} ${${COMPONENT}_SHARED_LIB}) 
        endif()
        
        # Set the classpath for this component; for downstream use.    
        set(${COMPONENT}_CLASSPATH ${FIBER_BUNDLES_CLASSPATH}:${CMAKE_ARCHIVE_OUTPUT_DIRECTORY}/${${COMPONENT}_JAVA_BINDING_JAR} CACHE STRING "Cumulative classpath for ${PROJECT_NAME}")
        ...
        add_dependencies(${${COMPONENT}_PYTHON_BINDING_LIB} ${FIBER_BUNDLES_PYTHON_BINDING_LIB} ${${COMPONENT}_SHARED_LIB})
