// $RCSfile: write_dof_tuple.t.cc,v $ $Revision: 1.20 $ $Date: 2012/03/01 00:41:23 $
...
main(int xargc, char* xargv[])
{
  ...
  sheaves_namespace* ns =
    new sheaves_namespace("write_dof_tuple.t");
  ...
  storage_agent sa("write_dof_tuple.t.hdf", sheaf_file::READ_WRITE);
  ...
  // Write the dof tuples.

  index_iterator* itr =
    lfield->schema().get_decomposition(ldecomp.index());

  while(!itr->is_done())
  {
    sa.write_dof_tuple(*lfield, itr->item(), ldecomp.index());
    itr->next();
  }
  delete itr;

  // Write the rest of the sec_rep_space.
