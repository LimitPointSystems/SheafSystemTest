...
test_name_multimap_facet()
{
  ...
  print_header("Begin testing \"name_multimap\" facet:");

  // Make a default namespace and get read/write access
  // (need read write access in order to delete).

  namespace_poset lns("test_namespace_poset");
  lns.get_read_write_access();

  // Get an iterator for the namespace.

  subposet_member_iterator itr(lns.whole());

  // Create a name_multimap of type T.

  name_multimap<T> lmap;

  // Iterate over the namespace poset putting the names into the map.

  print_subheader("Inserting map entries");

  int lnum_entries = 0;
  while(!itr.is_done())
  {
    // Every index can have more than one name,
  ...
  // Force index lnum_entries to have 3 names.

  const string lnames3[3] = {"alpha", "beta", "gamma"};
  for(int i=0; i<3; ++i)
  {
    cout << "Inserting \"" << lnames3[i] << "\" into the map at index: "
         <<  T(lnum_entries) << endl;
    lmap.put_entry(T(lnum_entries), lnames3[i], false);
  }
  lnum_entries++;

  // Test "put_entry(const entry_type& xentry, bool xunique)":

  print_subheader("Test \"put_entry(const entry_type& xentry, bool xunique)\"");

  typename sheaf::name_multimap<T>::entry_type lentry(T(lnum_entries), "zeta");
  cout << "Inserting \"zeta\" into the map at index: " <<  T(lnum_entries) << endl;
  lmap.put_entry(lentry, true);

  // Test "contains_entry" function:

  print_subheader("Test \"contains_entry(lentry)\"");

  bool ltest = lmap.contains_entry(lentry);
  cout << endl;
  cout << "Test map contains entry "  << lentry.second << ": "
       << boolalpha << ltest << endl;
  cout << endl;
  ...
  print_subheader("Testing the assignment operator");

  name_multimap<T> lmap_assign = lmap;

  cout << "Contents of the assigned object:" << endl;
  cout << lmap_assign << endl;
  cout << endl;

  // Test the copy constructor:

  print_subheader("Testing the copy constructor");

  name_multimap<T> lmap_copy(lmap);

  cout << "Contents of the copied object:" << endl;
  cout << lmap_copy << endl;
  cout << endl;

  // Compare the original with the copy for equality.

  bool ltest_copy = (lmap == lmap_copy);
  cout << endl;
  cout << "Test original==copy: " << boolalpha << ltest_copy << endl;
  cout << endl;
  ...
  for(int i=0; i<lct; i++)
  {
    cout << "Testing -- name is: " << lmap.name(i) << endl;
    cout << "Index " << i << " has " << lmap.name_ct(i)
         << " name(s) associated with it." << endl;
  }

  // Test "contains_name" function:

  cout << endl;
  cout << "Test map contains name \"beta\": " << lmap.contains_name("beta")
       << endl;
  cout << endl;

  // Test "index" function:

  print_subheader("Test \"index\" function");
  T lindex = lmap.index("beta");
  cout << "lindex = " << lindex << endl;
  cout << endl;

  // Test "all_names" function:

  print_subheader("Test \"all_names\" function");
  block<string> lnames;
  lmap.all_names(lindex, lnames);
  cout << "lnames = " << lnames << endl;
  cout << endl;

  // Test "name(index_type xindex, int xi)" function:

  print_subheader("Test \"name\" function");
  string lname = lmap.name(lindex, 2);
  cout << "lname = " << lname << endl;
  cout << endl;

  // Delete some names.

  // A namespace_poset always has a member called "bottom";
  // so it's safe to delete it.

  print_subheader("Deleting name \"bottom\"");

  lmap.delete_name("bottom");

  cout << lmap << endl;

  print_subheader("Deleting name \"beta\"");

  lmap.delete_name("beta");

  cout << lmap << endl;

  // Delete index 5.

  print_subheader("Deleting index \"5\"");

  if(lmap.contains_index(T(5)))
  {
    lmap.delete_index(T(5));
  }

  cout << lmap << endl;

  // Test "name_multimap(const string xnames[], size_type xnames_ub)":

  print_subheader("Test \"name_multimap(const string xnames[], size_type xnames_ub)\":");

  const string lnames_array[3] = {"one", "two", "three"};
  name_multimap<T> lmap3(lnames_array, 3);
  cout << lmap3 << endl;
  cout << endl;

  // Test "delete":

  print_subheader("Test delete \"~name_multimap()\":");

  name_multimap<T>* lmap86 = new name_multimap<T>(lmap);
  cout << *lmap86 << endl;
  delete lmap86;

  name_multimap<T>* lmap87 = new name_multimap<T>;
  cout << *lmap87 << endl;
  delete lmap87;
  ...
  print_footer("End testing \"name_multimap\" facet:");
...
test_name_map_facet()
{
  ...
  print_header("Begin testing \"name_map\" facet:");
  ...
  // Test "put_entry(const entry_type& xentry)":

  print_subheader("Test \"put_entry(const entry_type& xentry)\"");

  typename sheaf::name_map<T>::entry_type lentry(T(lnum_entries), "zeta");
  cout << "Inserting \"zeta\" into the map at index: " <<  T(lnum_entries)
       << endl;
  lmap.put_entry(lentry);
  ...
  print_subheader("Test \"ct()\" function");
  ...
  print_subheader("Test \"contains_name\" function");
  ...
  // Test non constant versions of "begin()" and "end()":

  typename sheaf::name_map<T>::iterator litr_begin = lmap.begin();
  typename sheaf::name_map<T>::iterator litr_end = lmap.end();
  typename sheaf::name_map<T>::iterator litr = litr_begin;
  for(litr = litr_begin; litr != litr_end; litr++)
  {
    cout << "index: " << litr->first << "\tname: " << *(litr->second) << endl;
  }

  // Test "name_map(const string xnames[], size_type xnames_ub)":

  print_subheader("Test \"name_map(const string xnames[], size_type xnames_ub)\":");

  const string lnames_array[3] = {"one", "two", "three"};
  name_map<T> lmap3(lnames_array, 3);
  cout << lmap3 << endl;
  cout << endl;

  // Test "delete_index(T):

  print_subheader("Test \"delete_index(T)\"");

  lmap3.delete_index(T(1));
  cout << lmap3 << endl;
  cout << endl;

  // Test "delete_name(const string& xname)":

  print_subheader("Test \"delete_name(const string& xname)\"");

  lmap3.delete_name("one");
  cout << lmap3 << endl;
  cout << endl;
  ...
  print_footer("End testing \"name_map\" facet:");
